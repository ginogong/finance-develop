...
def sma_strategy(stock_data, short_ma=5,long_ma=30):
	...
	return stock_data[['open','close', 'change','postion']]
...
long_num = np.arange(5,40)

short_num1 = np.arange(90,140) * -0.001
short_num = np.arange(2,15)
name = 1
for num2 in long_num:
	for num1 in short_num:
		df_zz = sma_strategy(df_zz,short_ma=num1,long_ma=num2)
		...
		date_list.append(num2)
		threshold_list.append(num1)
		print 'long:',num2,'short:',num1
df_result = pd.DataFrame({'start':start_date_list,'end':end_date_list,'last_days':last_days_list,\
						  'max_dd':max_dd_list,'return':total_return,'long':date_list,'short':threshold_list})
df_result = df_result[['long','short','start','end','last_days','max_dd','return']]
df_result.to_csv('zzshieldmacd002.csv')
