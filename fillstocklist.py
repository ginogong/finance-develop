...
from sqlalchemy import create_engine,Table,MetaData, Column, ForeignKey, Integer, String,DateTime
...
for item in data('#quotesearch ul a'):
	if pattern.findall(pq(item).text())[0][0:3] in ['600', '603', '601', '300', '002', '000']:
		...
		if not session.query(Stock_list).filter(Stock_list.code == sl.code):
			session.add(sl)
session.flush()
session.commit()
