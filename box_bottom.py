...
def compute(df,last_day):      # 计算函数
    if len(df) > 0:             #先判断df是否为空
        ...
        if last_trade_day == last_day:
            ...
            means = 0 
    flag = red_three(df)
    return pos_ratio,box_ratio,code,means,flag
...
def box_bottom(dataframe):
    ...
    dataframe = dataframe[(dataframe['box_ratio'] >box_lower)&(dataframe['box_ratio'] <box_upper)]
    dataframe = dataframe[dataframe['mean'] >means ]
    dataframe = dataframe[dataframe['flag']>1]
    return dataframe
...
def red_three(df):
    ...
    if len(df)>3:    
        ...
        df['inday_change'] = df['close'] - df['open']
        df_change = df[-3:]       
        df_change.index = range(3)
        #print df_change
        for i in range(3):
            if (0< df_change.loc[i,'change']<0.02) and (df_change.loc[i,'inday_change'] > 0):
                flag += 1
            else:
                flag -= 1
...
df_temp = df[df['code']=='300288']
#print red_three(df_temp)

#print df_temp
#print compute(df_temp,last_day)
df_re= df_groups.apply(compute,last_day)
index = df_re.index
df_re = pd.DataFrame(df_re.tolist(),columns=['position_ratio','box_ratio','code','mean','flag'])
df_re.index = index
