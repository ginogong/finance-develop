...
url_cls = 'http://www.cailianpress.com/'
...
def get_eastmoney_info():
    ...
    print 'getting  information from eastmoney has finished!'

def get_cls_info():
    url_cls = 'http://www.cailianpress.com/'
    df1 = pd.read_hdf(PATH + 'information.h5','info')
    response = requests.get(url=url_cls,headers=HEADER_REQ)
    response.encoding = 'utf-8'
    data = pq(response.text)
    pattern_title = re.compile(u'【.+】')
    pattern_content = re.compile(u'】.+')
    ...
    content_list = []
    now = datetime.datetime.now().strftime('%Y-%m-%d')
    for item in data('#entries ul li.content'):
        
        title = pattern_title.findall(pq(item).text())
        content = pattern_content.findall(pq(item).text())
        if len(content) >0:
            content_list.append(content[0][1:])
            title_list.append(title[0][1:-1])
    ...
    df1 = df1.drop_duplicates()
    df1.to_hdf(PATH + 'information.h5','info')
    print 'getting  information from cailianpress has finished!'
